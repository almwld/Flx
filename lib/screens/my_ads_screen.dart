...
class _MyAdsScreenState extends State<MyAdsScreen> {
  List<ProductModel> _ads = []; bool _isLoading = true; String _filter = 'الكل';
  final List<String> _filters = ['الكل', 'نشط', 'منتهي'];

  @override void initState() { super.initState(); _loadAds(); }
  Future<void> _loadAds() async {
    setState(() => _isLoading = true);
    final user = SupabaseService.currentUser;
    if (user != null) _ads = await SupabaseService.getSellerProducts(user.id);
    setState(() => _isLoading = false);
  }

  List<ProductModel> get _filteredAds {
    if (_filter == 'الكل') return _ads;
    return _ads.where((a) => (_filter == 'نشط' && a.status == 'active') || (_filter == 'منتهي' && a.status != 'active')).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(title: 'إعلاناتي', actions: [
        IconButton(icon: const Icon(Icons.add), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AddAdScreen())).then((_) => _loadAds()))
      ]),
      body: _isLoading ? const Center(child: CircularProgressIndicator()) : Column(children: [
        Container(height: 50, margin: const EdgeInsets.symmetric(vertical: 8), child: ListView.builder(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 16),
          itemCount: _filters.length,
          itemBuilder: (_, i) { final f = _filters[i]; final isSelected = _filter == f;
        ...
        Expanded(child: _filteredAds.isEmpty ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          ListView.builder(padding: const EdgeInsets.all(16), itemCount: _filteredAds.length,
            itemBuilder: (_, i) { final a = _filteredAds[i];
              return Card(margin: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.all(12),
                child: Row(children: [
                  ClipRRect(borderRadius: BorderRadius.circular(8), child: CachedNetworkImage(imageUrl: a.images.isNotEmpty ? a.images.first : '', width: 80, height: 80, fit: BoxFit.cover)),
                  const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(a.title, style: const TextStyle(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 4), Text('${a.price.toStringAsFixed(0)} ${a.currencySymbol}', style: const TextStyle(color: AppTheme.goldColor)),
