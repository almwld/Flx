...
class _AddAdScreenState extends State<AddAdScreen> {
  ...
  Future<void> _pickImages() async {
    ...
    if (picked != null) {
      setState(() => _images = picked.map((f) => File(f.path)).toList());
    }
  }
  ...
  Future<void> _submit() async {
    if (_images.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('أضف صورة على الأقل'), backgroundColor: AppTheme.error),
      );
      return;
    }
    setState(() => _isLoading = true);
    try {
      List<String> urls = await SupabaseService.uploadMultipleImages(
        filePaths: _images.map((f) => f.path).toList(),
        bucket: 'products',
      );
